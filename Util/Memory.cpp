...
PVOID MEM::PatternScan(PVOID pModule, const char* szPattern, int iOffset)
{
    ...
    __int32 ScanSize = ntHeaders->OptionalHeader.SizeOfImage - PatternBytesSize;

    // Scan
    for (__int32 i = 0; i < ScanSize; i++)
    {
        bool Found = true;
        for (__int32 j = 0; j < PatternBytesSize; ++j)
        {
...
PVOID MEM::PatternScan(const char* szPattern, int iOffset)
{
    ...
    while (true)
    {
            ...
            ((mbi.Protect & PAGE_EXECUTE_READWRITE) || (mbi.Protect & PAGE_EXECUTE_READ) || (mbi.Protect & PAGE_READONLY)))
        {
            ...
            {
                // Scan
                for (__int32 i = 0; i < mbi.RegionSize - PatternBytesSize; i++)
                {
                    bool Found = true;
                    for (__int32 j = 0; j < PatternBytesSize; j++)
                    {
